  ...
  return (
    <div className="all-bookmarks-container">
      <div className="search">
        ...
        <input className="search-input" type="text" value={searchedBookmark} onChange={updateSearchBookmark}
          placeholder="Search"></input>
      </div>
      {wantedBookmarks.map((bookmark) => {
        return (
          <MyBookmark key={bookmark._id} bookmark={bookmark}
            deleteBookmark={(bookmarkID) => { dataManager.deleteBookmark(bookmarkID, setMyBookmarks) }}
          />)
      })}
    </div>
  );
